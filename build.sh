...
do
    ...
    if [ "${GRUB4DOS_VER/EFI}" = "${GRUB4DOS_VER}" ]; then
        if [ -d $ACTION_PATH/grubutils/g4dext/bin ]; then
            7z a $GRUB4DOS_BIN $ACTION_PATH/grubutils/g4dext/bin
            7z rn $GRUB4DOS_BIN bin ext
        fi
    else
        if [ -d $ACTION_PATH/grubutils/g4eext/bin ]; then
            7z a $GRUB4DOS_BIN $ACTION_PATH/grubutils/g4eext/bin
            7z rn $GRUB4DOS_BIN bin ext
        fi
    fi
